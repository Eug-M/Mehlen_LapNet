...
path <- "/home/eugenie-modolo/Documents/Lapnet/donnees_KRAS/GSE303105_RAW/single_cell_QC_analysis"
setwd(path)
...
# seurat_obj$sample_id <- "GSM9117970_KPPC"
# seurat_obj$sra_id <- "SRX29782230"
seurat_obj$sample_id <- "GSM9117971_KPPC"
seurat_obj$sra_id <- " 	SRX29782231"
seurat_obj$condition <- sample_name
seurat_obj$patient_id <- paste0("Mouse_", sample_name)
...
# to check the names of these genes (mouse vs human): 
# rownames(seurat_obj@assays$RNA$counts)[grep("mt", rownames(seurat_obj@assays$RNA$counts), ignore.case = T)]
...
# nfeature_max <- 6000   # Maximum genes (above this = likely doublets)
...
nfeature_max <- 6000   # Maximum genes (above this = likely doublets)
ncount_min <- 300      # Minimum UMI per cell
ncount_max <- 30000    # Maximum UMI (above this = likely doublets)
mt_thresh <- 25        # Maximum mitochondrial % (adjust based on tissue)
...
# Control : Clu, Chil3, Dcn, S100a9, Col1a2, Igfbp7, Sparc, Col1a1, S100a8, Saa3 
# RMC6236 : Igha, S100a8, Clu, S100a9, Retnlg, Mmp7, Lcn2, Tnc, Dmbt1, Spp1
